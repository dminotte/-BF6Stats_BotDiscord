...
# --- CONFIG ---
TOKEN = "TON_TOKEN_DISCORD_ICI"  # <-- Mets ton token Discord ici
PREFIX = "!"
bot = commands.Bot(command_prefix=PREFIX, intents=discord.Intents.default())

# --- FONCTIONS UTILES ---
def load_font(font_list, size):
    """Charge la première police trouvée dans la liste ou arial par défaut."""
...
# --- FONCTION DE GÉNÉRATION DE BANNIÈRE ---
def generate_banner(player_name, platform="pc"):
    # --- APPEL API ---
    ...
    if not data.get("hasResults", False):
        raise Exception("Stats introuvables pour ce joueur.")
    ...
    player_font = load_font(["segoeui.ttf", "arial.ttf", "Tahoma.ttf"], 28)
    ...
    # --- FROSTED GLASS DERRIÈRE LES STATS ---
...
# --- COMMANDE DISCORD ---
@bot.command()
async def bf6stats(ctx, player_name: str, platform: str = "pc"):
    """Génère la bannière BF6 pour un joueur et une plateforme donnée."""
    platform = platform.lower()
    valid_platforms = ["pc", "xboxone", "ps4", "xboxseries", "ps5", "xbox", "psn"]
    if platform not in valid_platforms:
        await ctx.send(f"❌ Plateforme invalide. Valeurs possibles : {', '.join(valid_platforms)}")
        return

    await ctx.send(f"🎮 Génération des stats pour `{player_name}` sur `{platform}`...")
    try:
        banner_bytes = generate_banner(player_name, platform)
        await ctx.send(file=discord.File(fp=banner_bytes, filename='bf6_banner.png'))
    except Exception as e:
